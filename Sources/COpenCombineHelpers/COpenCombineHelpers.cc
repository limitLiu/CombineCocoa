...
// std::terminate will print the type and the error message of the in-flight
// exception.
#define OPENCOMBINE_HANDLE_EXCEPTION_END                                       \
  }                                                                            \
  catch (...) {                                                                \
    std::terminate();                                                          \
  }
...
// Throw an exception if the argument is non-zero with filename and line where
// the error occured.
...
protected:
  ...
  public:
    ...
    const pthread_mutexattr_t *raw() const noexcept { return &attrs_; }

    void setRecursive() { setType(PTHREAD_MUTEX_RECURSIVE); }

    void setErrorCheck() { setType(PTHREAD_MUTEX_ERRORCHECK); }

    ~Attributes() { pthread_mutexattr_destroy(&attrs_); }
  ...
  void initialize(const Attributes &attributes) {
    OPENCOMBINE_HANDLE_PTHREAD_CALL(
        pthread_mutex_init(&mutex_, attributes.raw()));
  }
};
...
class OS_UNFAIR_LOCK_AVAILABILITY OSUnfairLock final
    : PlatformIndependentMutex {
...
public:
  ...
  void lock() override { os_unfair_lock_lock(&mutex_); }

  void unlock() override { os_unfair_lock_unlock(&mutex_); }

  void assertOwner() override { os_unfair_lock_assert_owner(&mutex_); }
};
#endif // __APPLE__

template <typename Mu> class GenericMutex final : PlatformIndependentMutex {
  Mu mutex_;
...
OpenCombineUnfairRecursiveLock opencombine_unfair_recursive_lock_alloc(void) {
  OPENCOMBINE_HANDLE_EXCEPTION_BEGIN
  // TODO: Use os_unfair_recursive_lock on Darwin as soon as it becomes public
  // API.
...
void opencombine_unfair_recursive_lock_lock(
    OpenCombineUnfairRecursiveLock lock) {
  OPENCOMBINE_HANDLE_EXCEPTION_BEGIN
  static_cast<PlatformIndependentMutex *>(lock.opaque)->lock();
  OPENCOMBINE_HANDLE_EXCEPTION_END
}

void opencombine_unfair_recursive_lock_unlock(
    OpenCombineUnfairRecursiveLock mutex) {
  OPENCOMBINE_HANDLE_EXCEPTION_BEGIN
  static_cast<PlatformIndependentMutex *>(mutex.opaque)->unlock();
  OPENCOMBINE_HANDLE_EXCEPTION_END
}
...
void opencombine_unfair_recursive_lock_dealloc(
    OpenCombineUnfairRecursiveLock lock) {
  return delete static_cast<PlatformIndependentMutex *>(lock.opaque);
}
